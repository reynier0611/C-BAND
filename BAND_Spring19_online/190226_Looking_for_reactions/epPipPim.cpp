...
int main(int argc, char** argv) {
	...
	while(reader.next()==true){
		...
		// Proton variables
		int nProtons = 0;
		int tmp_fast_p_idx  = 0;	// index of the proton
		...
		// pi- variables
		int nPim = 0;
		int tmp_fast_pim_idx  = 0;	// index of the pi-
		...
		if(nProtons==1&&nPip==1&&nPim==1&&nParticles==4) {
			...
			TVector3 V3_p1v  = particles.getV3v  (tmp_fast_p_idx);		// proton vertex vector [cm]
			...
			TLorentzVector V4_p1p, V4_pipp, V4_pimp;
			...
			h1_dlt_vz_epip -> Fill(V3_pipv.Z()  - V3_ev.Z() );
	...
	c21 -> Print("results_epPipPim.pdf" );
	c22 -> Print("results_epPipPim.pdf" );
	c23 -> Print("results_epPipPim.pdf)");
