...
int main(int argc, char** argv) {
	...
	while(reader.next()==true){
		...
		if(nPip==1&&nParticles==2) {

			TVector3 V3_pipv  = particles.getV3v  (tmp_fast_pip_idx);		// pip 2 vertex vector [cm]
			TVector3 V3_pipp  = particles.getV3P  (tmp_fast_pip_idx);		// pip 2 momentum vector [GeV]
			float beta_pip    = particles.getBeta (tmp_fast_pip_idx);		// pip 2 beta = v/c

			TLorentzVector V4_pipp;

			V4_pipp.SetXYZM( V3_pipp.X() , V3_pipp.Y() , V3_pipp.Z() , mPiC );

			double EPip  = V4_pipp .E();

			// Missing momentum components
			TVector3       V3_Pm = V3_pipp - V3_q;
			TLorentzVector V4_Pm = V4_Ebeam + V4_mtar - V4_ep - V4_pipp;	
			...
			h1_dlt_vz_ep   -> Fill(V3_pipp.Z()  - V3_ev.Z() );
			h1_p_px        -> Fill(V3_pipp.X()              );
			h1_p_py        -> Fill(V3_pipp.Y()              );
			h1_p_pz        -> Fill(V3_pipp.Z()              );
			h1_p_p         -> Fill(V3_pipp.Mag()            );
			h1_p_th        -> Fill(rad2deg*V3_pipp.Theta()  );
			h1_p_phi       -> Fill(rad2deg*V3_pipp.Phi()    );
			h1_pmx         -> Fill(V3_Pm.X()                );
			h1_pmy         -> Fill(V3_Pm.Y()                );
			h1_pmz         -> Fill(V3_Pm.Z()                );
			...
			h2_p_th_phi   -> Fill(rad2deg*V3_pipp.Phi(), rad2deg*V3_pipp.Theta());
	...
	c0  -> Print("results_ePip.pdf(");
	c1  -> Print("results_ePip.pdf" );
	c2  -> Print("results_ePip.pdf" );
	c3  -> Print("results_ePip.pdf" );
	c5  -> Print("results_ePip.pdf" );
	c6  -> Print("results_ePip.pdf" );
	c7  -> Print("results_ePip.pdf" );
	c8  -> Print("results_ePip.pdf" );
	c9  -> Print("results_ePip.pdf" );
	c10 -> Print("results_ePip.pdf" );
	c11 -> Print("results_ePip.pdf" );
	c12 -> Print("results_ePip.pdf" );
	c13 -> Print("results_ePip.pdf" );
	c14 -> Print("results_ePip.pdf" );
	c15 -> Print("results_ePip.pdf" );
	c16 -> Print("results_ePip.pdf" );
	c17 -> Print("results_ePip.pdf" );
	c18 -> Print("results_ePip.pdf" );
